...
class Device extends EventEmitter {
    constructor(_interface, _changedInterfaceFactory) {
        super();
        ...
        // Only create the DBus interface _changed once someone actially subscribes to the events/signals it emits
        // Passing in a factory into this constructor instead of the _changed interface directly
        this.on('newListener', (async (eventName) => {
            if ('PropertiesChanged' === eventName && !this._changed) {
                this._changed = true; // set to true for now so the factory is called only once in case of multiple events
                this._changed = await _changedInterfaceFactory();
                this._changed.on('PropertiesChanged', function(iface, propertiesAdded, propertiesRemoved) {
                    this.emit('PropertiesChanged', this, iface, propertiesAdded, propertiesRemoved);
                }.bind(this));
            }
        }).bind(this));
    }
