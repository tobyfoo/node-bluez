...
class Bluez extends EventEmitter {
    ...
    async getDevice(address) {
        ...
        // Create an async function which will return a Promise to the device's Properties interface, but only if called
        // (This is done on demand only because we don't want the extra bus traffic if it's not needed by the Device object)
        const changedInterfaceFactory_ = ((bluez_, deviceObjectPath_) => async () => await bluez_.getInterface('org.bluez', deviceObjectPath_, 'org.freedesktop.DBus.Properties'))(this, this.devices[address]);
        // need to check here again, there might be a race because of await
        if (this.devices[address] && typeof this.devices[address] !== 'string') {
            // Device already created
            return this.devices[address];
        }
        this.devices[address] = new Device(interface_, changedInterfaceFactory_);
        return this.devices[address];
    }
